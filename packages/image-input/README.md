...
- On an idle, native `steer`, or native `followUp` interactive TUI `input` event, every canonical clipboard image path is read, validated by content signature, normalized with Pi's public `resizeImage()` defaults, replaced in place by a backtick-wrapped `[Image]` marker, and appended as a flat image attachment in occurrence order.
...
The backtick-wrapped `[Image]` marker records text position only. It does not provide a stable image identity and does not prevent a model from choosing to call tools for other reasons. Successful conversion means the image is already included in the same user message as native image content.

Queued image recovery follows Pi's native behavior. Dequeue, Esc, or abort can restore only the transformed backtick-wrapped `[Image]` text and lose the queued Image Attachments. Switching to a text-only model after queueing can cause Pi to replace an Image Attachment with its unsupported-image placeholder before the upstream provider request. This extension does not add recovery or model-switch state.
...
Normal installed-extension and isolated package loading were verified. Manual Windows TUI smoke passed in the normal installed-extension environment, including `pi-fff`. It covered idle and streaming `steer`/`followUp` delivery, multiple queued messages, Image Marker plus flat ImageContent session shape, accepted lossy dequeue behavior, active-compaction blocking, the unsupported-current-model guard, and the post-queue text-only-model boundary. WSL, X11/Wayland, macOS, and providers outside the tested Windows environment were not tested.
